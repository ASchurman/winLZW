                         ...
                         bool eFlag)
{
    ...
    table->array = (tableElt*)malloc(sizeof(tableElt) * table->arraySize);
    
    table->hashSize = (table->arraySize * 2) + 1;
    table->hash = (tableElt**)malloc(sizeof(tableElt*) * table->hashSize);
...
{
    // copy pi into oldPi
    pruneInfo* oldPi = (pruneInfo*)malloc(sizeof(pruneInfo));
    oldPi->counter = pi->counter;
    oldPi->lastSeen = (unsigned long*)malloc(sizeof(unsigned long) * table->arraySize);
...
{
    pruneInfo* pi = (pruneInfo*)malloc(sizeof(pruneInfo));
    pi->lastSeen = (unsigned long*)malloc(sizeof(unsigned long) * (1 << maxBits));
    pi->counter = 1;
